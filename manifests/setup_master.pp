class start_master::setup_master(
  ...
  # ---- OS handling ----
  case $distro {
    'RedHat': { $puser = 'nginx'; $pgroup = 'nginx' }
    'Debian': { $puser = 'www-data'; $pgroup = 'www-data' }
    default: { fail("Unsupported OS family: ${distro}") }
  }

  File { owner => $user, group => $group }

  # ---- 1-6: All PRE-webstack resources (unchanged, simplified) ----
  ...
  # ---- CRITICAL: CONTAINED WEBSTACK (breaks cycle) ----
  contain_webstack()

  # Define contained webstack class
  class contain_webstack() {
    # FIREWALL FIRST (RedHat)
    if $distro == 'RedHat' {
      service { 'firewalld':
        ensure => stopped,
        enable => false,
   ...
   ## contain 'php'
    ...
    # STEP 4: Nginx resources (after FPM pool)
    Php::Fpm::Pool[$fqdn] ->
